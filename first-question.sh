...
plotTitle='PER vs. Distancia'

# Packet Size - Modified for the 1st question
packetSize=128
...
for distance in `seq $minDistance $stepSize $maxDistance`; do
  ./waf --run "link-performance --packetSize=${packetSize} --frequency=${frequency} --maxPackets=${maxPackets} --transmitPower=${transmitPower} --noisePower=${noisePower} --distance=${distance} --metadata=${distance} --RngRun=${RngRun}"
done
...
/usr/bin/python ../utils/plot-lines-with-error-bars.py --title="${plotTitle}" --xlabel='Distancia (m)' --ylabel='Taxa de Erro de Pacote (PER)' --xcol=5 --ycol=3 --yerror=4 --fileName=link-performance-summary.dat --plotName=${plotName}
