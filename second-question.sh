...
for distance in `seq $minDistance $stepSize $maxDistance`; do
  ./waf --run "link-performance --frequency=${frequency} --maxPackets=${maxPackets} --transmitPower=${transmitPower} --noisePower=${noisePower} --distance=${distance} --metadata=${distance} --RngRun=${RngRun}"
done
